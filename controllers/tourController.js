...
// Uploading and processing tour images.
...
exports.resizeTourImages = async (req, res, next) => {
  ...
  // Buffer has the images in memory
...
exports.getTour = catchAsync(async (req, res, next) => {
  const tour = await Tour.findById(req.params.id)
    .populate('guides')
    .populate('reviews');
  // Factor it in a middleware in the tour model!
...
exports.getToursWithin = async (req, res, next) => {
  ...
  const radius = unit === 'mi' ? distance / 3963.1 : distance / 6378.1;
  ...
  const distances = await Tour.aggregate([
    {
      $geoNear: {
        ...
        // Here we set the name of the
        // property that will be created.
